...
import { ContentStatus, Evento } from '../../types';
import { CategoryBadge } from '../ui/CategoryBadge';

interface AdminEventosCrudProps {
  eventos: Evento[];
  onOpenCreate: () => void;
  onOpenEdit: (evento: Evento) => void;
  onOpenDelete: (id: string, title: string) => void;
  onToggleStatus: (id: string) => void;
}

export const AdminEventosCrud: React.FC<AdminEventosCrudProps> = ({
  eventos,
  onOpenCreate,
  onOpenEdit,
  onOpenDelete,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-display font-bold text-2xl text-white flex items-center gap-2">
            ...
            <span>Gerenciar Eventos da Agenda</span>
          </h2>
          <p className="text-xs text-neutral-400">
            Organize debates, mesas redondas, calouradas e atos convocados pelo CACISO.
          </p>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-500 text-white font-bold text-xs shadow-md transition-all self-start sm:self-auto"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-3 p-4 rounded-2xl bg-neutral-900 border border-neutral-800">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-xl bg-neutral-950 border border-neutral-800 text-xs text-white placeholder-neutral-500 focus:outline-none focus:border-teal-500"
          />
        </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-neutral-800 bg-neutral-950/80 text-neutral-400 uppercase tracking-wider font-semibold">
                <th className="py-3.5 px-4">Evento & Descrição</th>
            ...
            <tbody className="divide-y divide-neutral-800/80">
              {filtered.map((item) => (
                <tr key={item.id} className="hover:bg-neutral-800/30 transition-colors">
                  ...
                  <td className="py-3.5 px-4 text-neutral-400 max-w-[180px] truncate">
                    {item.local}
                  </td>
                  <td className="py-3.5 px-4 whitespace-nowrap">
                    <button
